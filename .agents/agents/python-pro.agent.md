...
tools: execute, read, edit, search, web/fetch, 'context7/*', todo
...
**Before writing code that uses an external library**, resolve its ID and fetch relevant docs:
1. Use `context7/resolve-library-id` with the library name
2. Use `context7/get-library-docs` with the resolved ID and a focused topic

If `context7/*` MCP tools are unavailable, fall back to the `.github/skills/context7` skill (REST API via `scripts/context7.sh`).
